...
class ClothRecommendAdapter(private val recommendClothList: ArrayList<ClothFooter>) : RecyclerView.Adapter<ClothRecommendAdapter.CustomViewHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ClothRecommendAdapter.CustomViewHolder {
    ...
    override fun getItemCount(): Int {
        return recommendClothList.size
        // 데이터 세트의 아이템 수를 반환합니다.
    }

    override fun onBindViewHolder(holder: CustomViewHolder, position: Int) {
        // onBindViewHolder 함수는 뷰 홀더에 데이터를 바인딩합니다.
        holder.image.setImageResource(recommendClothList[position].image)
        // 현재 위치의 프로필 이미지를 설정합니다.
        //holder.name.text = pantsClothList[position].name
        // 현재 위치의 이름을 설정합니다.
        holder.brand.text = recommendClothList[position].brand
        // 현재 위치의 브랜드를 설정합니다.
        holder.price.text = recommendClothList[position].price.toString()
    }
